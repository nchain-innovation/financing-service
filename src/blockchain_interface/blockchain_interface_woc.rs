...
mod tests {
    ...
    // address for test n1jaAsKZfE6kufLy5DAtAyQ1RzGXwMeNAF

    #[tokio::test]
    async fn test_get_balance() {
        let bci = get_blockchain_interface();

        let result = bci.get_balance("n1jaAsKZfE6kufLy5DAtAyQ1RzGXwMeNAF").await;
        dbg!(&result);
        assert!(&result.is_ok());
    }
    ...
    async fn test_get_utxo() {
        ...
        let result = bci.get_utxo("n1jaAsKZfE6kufLy5DAtAyQ1RzGXwMeNAF").await;
        dbg!(&result);
        assert!(&result.is_ok());
    }
}
