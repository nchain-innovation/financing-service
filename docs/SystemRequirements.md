...
| SR-FUNC-002 | The service SHALL expose `GET /health` returning HTTP 200 and JSON `{"status":"ok"}` when no mapi-lite is configured (see SR-BCHN-011 for the mapi-lite case). | Must | **AT:** `rest_api::tests::test_health`, `responses::tests::health_response_without_mapi_lite_is_exactly_status_ok` |
...
| SR-BCHN-004 | `/health` SHALL NOT depend on blockchain (read) interface connectivity. Without `[mapi_lite]` it is liveness only; with `[mapi_lite]` it reflects mapi-lite reachability (SR-BCHN-011). | Must | **AT:** `rest_api::tests::test_health`, `service::tests::sr_bchn_011_mapi_lite_health_is_none_without_mapi_lite` |
...
| SR-BCHN-011 | When `[mapi_lite]` is configured, `GET /health` SHALL probe mapi-lite (`GET /mapi/feeQuote`, bounded by `health_timeout_seconds`) and SHALL return HTTP 503 with `status: "unhealthy"` when the probe fails. The verdict SHALL be cached for `health_timeout_seconds`, so this unauthenticated and rate-limit-exempt endpoint (SR-SEC-013) cannot be used to flood mapi-lite, and the 503 body SHALL carry a generic detail rather than the upstream error. An unreachable mapi-lite at startup SHALL be warned about but SHALL NOT stop the service, so the read paths stay up and the service recovers when mapi-lite returns. | Must | **AT:** `rest_api::tests::sr_bchn_011_health_reports_mapi_lite_ok_when_it_is_reachable`, `sr_bchn_011_health_returns_503_when_mapi_lite_is_unreachable`, `service::tests::sr_bchn_011_mapi_lite_health_reports_the_probe_result`, `sr_bchn_011_mapi_lite_health_is_cached_for_the_health_timeout`, `broadcaster::mapi::tests::sr_bchn_011_mapi_broadcaster_health_check_passes_on_a_fee_quote`, `sr_bchn_011_mapi_broadcaster_health_check_fails_on_an_error_status` |
