...
| SR-BCHN-009 | Transaction broadcasting SHALL sit behind a `TxBroadcaster` seam with two implementations. When `[mapi_lite]` is configured, funding transactions SHALL be broadcast via mapi-lite (`POST /mapi/txs`) and `GET /status` SHALL report `broadcaster: "mapi-lite"`; otherwise they SHALL be broadcast via the configured blockchain interface, unchanged from before. Chain reads SHALL use the blockchain interface in both cases. | Must | **AT:** `broadcaster::factory::tests::sr_bchn_009_factory_selects_the_blockchain_interface_without_mapi_lite`, `sr_bchn_009_factory_selects_mapi_lite_when_configured`, `broadcaster::mapi::tests::sr_bchn_009_mapi_broadcaster_posts_the_transaction_to_mapi_txs`, `service::tests::sr_bchn_009_execute_funding_uses_the_broadcaster_not_the_blockchain_interface`, `sr_bchn_009_fund_with_multiple_transactions_uses_the_broadcaster`, `rest_api::tests::sr_bchn_009_status_names_mapi_lite_and_fund_broadcasts_through_it`, `system_requirements::tests::sr_bchn_009_uls_client_and_uls_core_are_pinned_to_one_mapi_lite_revision`, `sr_bchn_009_configuration_and_readme_document_mapi_lite` |
...
| SR-BCHN-011 | When `[mapi_lite]` is configured, `GET /health` SHALL probe mapi-lite (`GET /mapi/feeQuote`, bounded by `health_timeout_seconds`) and SHALL return HTTP 503 with `status: "unhealthy"` when the probe fails; the service SHALL also refuse to start when mapi-lite is unreachable. | Must | **AT:** `rest_api::tests::sr_bchn_011_health_reports_mapi_lite_ok_when_it_is_reachable`, `sr_bchn_011_health_returns_503_when_mapi_lite_is_unreachable`, `service::tests::sr_bchn_011_mapi_lite_health_reports_the_probe_result`, `broadcaster::mapi::tests::sr_bchn_011_mapi_broadcaster_health_check_passes_on_a_fee_quote`, `sr_bchn_011_mapi_broadcaster_health_check_fails_on_an_error_status` |
...
| SR-CFG-008 | The `[mapi_lite]` section SHALL be optional; a config without it SHALL load as before. When present, `base_url` SHALL be required and validated (`http://` or `https://`), timeouts SHALL be non-zero, `timeout_seconds`/`health_timeout_seconds`/`max_retries` SHALL default to 30/2/2, and the section SHALL also be accepted inside `FS_CONFIG` JSON. | Must | **AT:** `config::tests::sr_cfg_008_config_without_a_mapi_lite_section_has_none`, `sr_cfg_008_mapi_lite_section_parses_with_defaults`, `sr_cfg_008_mapi_lite_section_accepts_every_field`, `sr_cfg_008_mapi_lite_validate_rejects_a_missing_or_non_http_base_url`, `sr_cfg_008_mapi_lite_validate_rejects_zero_timeouts`, `sr_cfg_008_load_config_validates_the_mapi_lite_section`, `sr_cfg_008_load_config_reads_a_valid_mapi_lite_section`, `sr_cfg_008_fs_config_json_accepts_a_mapi_lite_object` |
